...
def main():
    ...
    for idx, row in enumerate(records):
        ...
        print(f"  GT:     '{gt}'")
        print(f"  Legacy: '{text_legacy}' (similarity: {sim_legacy:.2%}, conf: {conf_legacy:.2%})")
        print(f"  HSV:    '{text_hsv}' (similarity: {sim_hsv:.2%}, conf: {conf_hsv:.2%})")
        print()

    print("=" * 60)
    print("COMPARATIVE TROCR ACCURACY RESULTS")
    print("=" * 60)
    print(f"Average Similarity (Legacy): {np.mean(legacy_scores):.2%}")
    print(f"Average Similarity (HSV):    {np.mean(hsv_scores):.2%}")
    print("-" * 60)
    print(f"High-Accuracy matches (>= 95% similarity) under Legacy: {legacy_exact} / {len(records)} ({legacy_exact/len(records):.2%})")
    print(f"High-Accuracy matches (>= 95% similarity) under HSV:    {hsv_exact} / {len(records)} ({hsv_exact/len(records):.2%})")
    print("=" * 60)
