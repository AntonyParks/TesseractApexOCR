...
def main():
    ...
    with csv_path.open("r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if filepath.exists() and row["label"].strip():
                # Filter for mostly English labels
                if not any(ord(c) > 127 for c in row["label"] if c not in "✔️"):
                    records.append(row)
                    
    print(f"Found {len(records)} matching records in labeled dataset.")
    if not records:
        return
    ...
    print(f"Comparing preprocessing methods on {sample_size} sample crops...\n")
    ...
    for idx, row in enumerate(sample):
        ...
        text_legacy = pytesseract.image_to_string(legacy_img, config=TESS_CONFIG).strip()
        text_hsv = pytesseract.image_to_string(hsv_img, config=TESS_CONFIG).strip()
        ...
        if idx < 5:
            print(f"Crop {idx+1} ({Path(row['filepath']).name}):")
            print(f"  GT:     '{gt}'")
            print(f"  Legacy: '{text_legacy}' (similarity: {sim_legacy:.2%})")
            print(f"  HSV:    '{text_hsv}' (similarity: {sim_hsv:.2%})")
            print()

    print("=" * 60)
    print("COMPARATIVE PREPROCESSING ACCURACY RESULTS")
    print("=" * 60)
    print(f"Average Similarity (Legacy): {np.mean(legacy_scores):.2%}")
    print(f"Average Similarity (HSV):    {np.mean(hsv_scores):.2%}")
    print("-" * 60)
    print(f"High-Accuracy matches (>= 95% similarity) under Legacy: {legacy_exact} / {sample_size} ({legacy_exact/sample_size:.2%})")
    print(f"High-Accuracy matches (>= 95% similarity) under HSV:    {hsv_exact} / {sample_size} ({hsv_exact/sample_size:.2%})")
    print("=" * 60)
